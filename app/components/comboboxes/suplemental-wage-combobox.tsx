...
export function SuplementalWageComboBox({
  wages,
  name,
  errorMessage,
  defaultValue,
}: Props) {
  const [wageNameId, setWageNameId] = useState<Key | null>();

  return (
    <>
      <input type="hidden" name={name} value={wageNameId || ""} />
      <RAComboBox
        label="Suplemental Wage"
        defaultItems={wages}
        onSelectionChange={setWageNameId}
