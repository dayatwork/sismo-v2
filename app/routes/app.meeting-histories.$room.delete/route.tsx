...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (!roomName) {
    return redirect(`/app/meeting-histories`);
  }

  await requirePermission(request, "manage:meeting");

  await deleteClosedMeetingByRoomName({ roomName });
...
export default function DeleteMeetingRoom() {
  ...
  return (
    <Modal
      isDismissable
      isOpen={true}
      onOpenChange={() => navigate(`/app/meeting-histories`)}
        ...
        <Form method="post">
          ...
          <div className="mt-4 flex justify-end gap-2 w-full">
            <Button
              type="button"
              variant="ghost"
              onClick={() => navigate(`/app/meeting-histories`)}
            >
              Cancel
            </Button>
            <Button
