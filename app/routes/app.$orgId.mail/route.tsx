...
export async function loader({ params, request }: LoaderFunctionArgs) {
  ...
  const [inboxMailCount, sentMailCount, draftMailCount] = await Promise.all([
    getInboxMailCount({ organizationId, receiverId: loggedInUser.id }),
    getSentMailCount({ organizationId, senderId: loggedInUser.id }),
    getDraftMailCount({ organizationId, senderId: loggedInUser.id }),
  ]);
