...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (!userId) return redirect(`/app/${organizationId}/employees`);
  ...
  console.log({ divisionId, jobLevelId });
  ...
  return redirectWithToast(`/app/${organizationId}/employees/${userId}`, {
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  if (!employeeId) return redirect(`/app/${organizationId}/employees`);
  ...
  const [directorates, jobLevels] = await Promise.all([
    getDirectorates(organizationId),
    getJobLevels(organizationId),
  ]);
...
export default function AddEmployeePosition() {
  ...
  return (
    <Modal
      isDismissable
      isOpen={true}
      onOpenChange={() => navigate(`/app/${orgId}/employees/${id}`)}
        ...
        <Form method="post" id={form.id} onSubmit={form.onSubmit}>
          ...
          <div className="mt-4 flex justify-end gap-2 w-full">
            <Button
              ...
              onPress={() => navigate(`/app/${orgId}/employees/${id}`)}
            >
              Cancel
            </Button>
            <Button
